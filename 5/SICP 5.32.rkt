...
(define (lookup-prim symbol operations)
  (let ((val (assoc symbol operations)))
    (if val
        (if (null? (cdr val))
            (error "Your mother" (list val symbol operations))
            (cadr val))
...
(define operations 
  (list (list 'true? true?)
        ...
        (list 'input-prompt input-prompt)
        (list 'output-prompt output-prompt)
...
(define registers '(exp env val continue proc argl unev))
(define eceval (make-machine registers operations exeval))
(define the-global-environment (setup-environment))
...
(define m (make-machine '(w) operations '(controller
                                          (assign w (const 4)))))
(set-register-contents! m 'w 5)
(start m)
(get-register-contents m 'w)

#|

We could do that, but it would make the evaluator more complex,
also, the parsing required would diminish the performance gains,
at least from the interpreter perspective.
