...
(
 #|
 This one, I believe is a direct approach, without using the let->combination procedure defined in previous
 chapters. Although we use the same approach, so we are basically defining let->combination here in machine
code rather than using the underlying scheme. I did not test if it works.
 |#
 eval-dispatch
 (test (op let?) (reg exp))
 (branch (label ev-let-ex))
 ev-let-ex
 (save proc)
 (save unev)
...
(
 #|
 This one is as the book meant it to be, letting all the work to underlying scheme.
 |#
 eval-dispatch
 (test (op let?) (reg exp))
