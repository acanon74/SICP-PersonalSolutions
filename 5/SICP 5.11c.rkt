  ...
  (let ((s '()))
    (define (push x r)
      ...
      (define (search l rn)
        ...
        (if (tagged-list? (car l) rn)
            
            (let ((rstack (cdar l)))
              (display  rstack)
              (set! rstack (cons x rstack))
              )
            (search (cdr l) rn)
            )
        )
      (search s (get-name r))
      ;(set! s (cons (cons r x) s))
      )
    ...
    (define (pop r)
      ...
      (define (search2 l rn)
        ...
        (if (tagged-list? (car l) rn)
            ...
            (let ((rstack (cdar l)))
              (if (null? rstack)
                  (error "Register stack empty -- POP" rn)
                  (let ((top (car rstack)))
                    (set! rstack (cdr rstack))
                    top)
                  )
      ...
      (if (null? s)
          (error "Empty main stack -- POP" )
          (search2 s (get-name r))
          )
      )

    
    (define (initialize lr)
      (set! s (map (lambda (r) (cons r '())) lr))
      (display s)
      'djhlone)
...
(define (initilize-stack stack lr)
  ((stack 'initialize) lr))
