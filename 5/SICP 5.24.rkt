...
(define (first-clause lclause)
  (car lclause))
(define (next-clause lclause)
  (cdr lclause))

(
#|
MY SOLUTION. I will try this once I write down the entire codebase.
|#
...
(goto (label loop-cond))

loop-cond

(save exp) ;list of clauses
(assign exp (op first-clause) (reg exp)) ;get first
(test (op cond-else-clause?) (reg exp))
(branch (label cond-else))
(assign unev (op cond-predicate) (reg exp)) ;predicate
(test (op true?) (reg unev))
(branch (label cond-true))
(restore exp) ;return to nonevaluated list
(assign exp (op next-clauses) (reg exp))
(goto (label loop-cond))
