...
(define (put op types val)
  (hash-set! op-table (list op types) val))
(define (get op types)
  (hash-ref op-table (list op types) #f))
...
(define type-levels (make-hash))
(define (put-type-level type level)
  (hash-set! type-levels type level))
(define (get-type-level type)
  (hash-ref type-levels type))
