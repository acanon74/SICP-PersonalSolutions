...
(define (triples a b c)
  ...
  (define (init p e)
    (cons-stream
     (append (stream-car p) (stream-car e))
   (interleave
    (stream-map (lambda (x) (append (stream-car p) x))
                (stream-cdr e))
    (init (stream-cdr p) (stream-cdr e))))
...
(define pytha (stream-filter (lambda (x)
                               (and (<= (tr-a x) (tr-b x))
                                    (= (+ (square (tr-a x)) (square (tr-b x)))
                                       (square (tr-c x))))) t))
...
(define (triples p e)
   ...
   (interleave
    (stream-map (lambda (x) (append (stream-car p) x))
                (stream-cdr e))
    (triples (stream-cdr p) (stream-cdr e))))
