...
#|
It doesn't affect the result since at the end it will test for all the conditions regardless of the order.

It will affect runtime, since we can choose to test for conditions with lower O time first. For example, testing
for an equality = is done in constant time while procedures such as distinct? will take more computation.
We would like to discard solutions as fast as possible so checking constant time procedure first will rule out
solutions which fail them faster, and, in the case in which they pass those, constant procedures will not have
a big accumulation impact to the runtime of the other tests.
|#
...
(define (multiple-dwelling)
  (let ((baker (amb 1 2 3 4 5))
        ...
        (smith (amb 1 2 3 4 5)))
    ; if require true then continue, if false quit
