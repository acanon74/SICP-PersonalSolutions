...
(define (allbut l x i)
  (cond ((null? l) '())
        ((= i x) (allbut (cdr l) x (+ i 1)))
        (else (cons (car l) (allbut (cdr l) x (+ i 1))))
        )
  )
  ...
  (let ((cprocs (map analyze (amb-choices exp))))
    (lambda (env succeed fail)
      ...
      (define (try-next choices)
        (if (null? choices)
            (fail)
            (let ((number (random-int 0 (- (length choices) 1))))
              ((list-ref choices number) env
                                         succeed
                                         (lambda ()
...
I will not run it because I do not have a prime? procedure. Regardless, p will pairs will collect all the prime pairs, which is
static since we are using permanentset. primesumpair will make sure they are prime pairs via its require, if they are its
success continuation is executing (amb). Will we can think of it as a force failure to move on to the next iteration.
After we have check every combination of the lists the require in an-element-of will fail,
triggering the global fail continuation and thereby, triggering if-fail which as we have defined it,
will test as failed and return the second expression, in this case it is to return pairs.
|#
